...
def run():
    ...
    for folder in itertools.chain(folders):
        ...
        fig_whole_type, axarr = plt.subplots(nrows=num_by_type, ncols=1, figsize=(24, 6*num_by_type))        
        for num, csv in enumerate(glob.glob(os.path.join(folder, '*', '*.csv'))):
            ...
            for idx, col_name in enumerate(df.columns):
                ...
                if idx == 0:
                    ...
                    ax.legend(loc=2, fancybox=True, framealpha=0.3, ncol=1)
                    if num_by_type <= 1:
                        df.plot(ax=axarr, x=df.keys()[0], y=df.keys()[1::], title=exp_name , legend=True)
                        axarr.legend(loc=2, fancybox=True, framealpha=0.3, ncol=1)                        
                        ...
                        df.plot(ax=axarr[num], x=df.keys()[0], y=df.keys()[1::], title=exp_name, legend=True)
                        axarr[num].legend(loc=2, fancybox=True, framealpha=0.3, ncol=1)                        
