...
def run():
    ...
    logger.info(folders)
    with open(os.path.join(datasets_of_filtering_schemes_folder, 'dataset_info.txt'), 'w') as dataset_info:
        anomalies = []
    ...
    with open(os.path.join(datasets_of_filtering_schemes_folder, 'dataset_info.txt'), 'r+') as f:
        file_data = f.read()
        f.seek(0,0)
        f.write(''.join(dtype) + '\n' + file_data)
